...
bool register_ipc_mem_handle(
	...
	if (context_end_layer <= decoding_start_layer || context_start_layer >= decoding_end_layer ||
		...
		// cudaError_t err2 = cudaIpcOpenMemHandle(&context_worker_v_cache_addr[context_worker_hash], v_cache_handle, cudaIpcMemLazyEnablePeerAccess);
...
bool register_ipc_mem_handle2(
	...
	if (decoding_end_layer <= context_start_layer || decoding_start_layer >= context_end_layer ||
		...
		cudaError_t err2 = cudaIpcOpenMemHandle(&decoding_worker_v_cache_addr[decoding_worker_hash], v_cache_handle, cudaIpcMemLazyEnablePeerAccess);
		fprintf(stderr, "CUDA error2:%s\n", cudaGetErrorString(err2));
